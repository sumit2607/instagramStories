...
import com.example.instagramstories.remote.roomdb.db.ResultModelDao
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext

class StoryRepository(
    private val storyDao: ResultModelDao,
    private val storyApiService: StoryApi // Assume you have an API service class
) {

    val allStories: List<DataModel>? = storyDao.getResponseModelByPage()
                ...
                if (response.isExecuted) {
                    response.execute().body()?.let { stories ->
                        ...
                        for (i in stories.listIterator()) {
                            storyDao.insertResultModel(stories) // Insert new data
                        }
